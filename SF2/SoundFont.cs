...
namespace GotaSoundBank.SF2
{
    ...
    public class SoundFont : IOFile
    {
        ...
        public override void Read(FileReader r2)
        {
            ...
            //Get preset bags.
            List<Tuple<ushort, ushort>> presetGenModIndices = new();
            List<Zone> presetZones = new();
            ...
            for (int i = 0; i < Presets.Count; i++)
            {
                ...
                {
                    if (Presets[i].Zones.Count == 0 && !presetZones[j].Generators.Where(x => x.Gen == SF2Generators.Instrument).Where(x => x.Gen == SF2Generators.Instrument).Any())
                    {
            ...
            //Get instrument bags.
            List<Tuple<ushort, ushort>> instrumentGenModIndices = new();
            List<Zone> instrumentZones = new();
            ...
            for (int i = 0; i < Instruments.Count; i++)
            {
                ...
                for (int j = startZone; j < startZone + numZones; j++)
                {
                    if (Instruments[i].Zones.Count == 0 && !instrumentZones[j].Generators.Where(x => x.Gen == SF2Generators.SampleID).Any())
                    {
        ...
        public override void Write(FileWriter w2)
        {
            ...
            Dictionary<SampleItem, long> samplePositions = new();
